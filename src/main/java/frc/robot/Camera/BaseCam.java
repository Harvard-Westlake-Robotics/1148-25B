...
public abstract class BaseCam {
  public class AprilTagResult {
    ...
    public AprilTagResult(
        Pose2d pose, double time, double distToTag, int tagCount, double ambiguity) {
      this.pose = pose;
      this.time = time;
  ...
  public boolean addVisionEstimate(
    ...
    if (oEstimation.isPresent()) {
      ...
      addVisionMeasurement.accept(estimation.pose, estimation.time, stdDeviations);
      return true;
    }
    return false;
  }
