...
public class ArmCommand extends Command {
  ...
  public void setHeight(ScoringLevel level) {
    ...
    if (level == ScoringLevel.NEUTRAL) {
      // Change this if you want to enable the "neutral" position that changes based on the robot's
      // current state.
      // I'm disabling by default just in case
      boolean neutral = true;
      if (neutral) {
        // DISCLAIMER: POSITIONS NEED TO BE TUNED BUT THESE VALUES WON'T BREAK ANYTHING
        // When intake has a coral hotdog style, the pivot and wrist are at in a general "scoring"
        // position
