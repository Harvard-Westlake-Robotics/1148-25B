...
public class PLog {
  ...
  /**
   * Log a failure which may kill one function or one thread, however the robot as a whole can keep
   * functioning.
   *
   * @param category
   * @param message
  ...
  /**
   * Log something which should not happen under normal circumstances and probably is a bug, but
  ...
  /**
   * Log a semi-important message which the user should probably see, but does not indicate anything
   * is broken.
   */
  ...
  /**
   * Log a message which is not important during normal operation, but is useful if you're trying to
   * debug the robot.
   *
   * @param category
   * @param message
